...
# Output: optimism-corrected C-index + calibration plots at 1/3/5y + PH checks + tables
...
# ----------------------------
# Load model and data artifacts from 05
# ----------------------------
fit <- readRDS("model/cox_simple_fit.rds")
df  <- readRDS("data/processed/analysis_df.rds")

# Recreate the same analysis frame used in 05 (must match exactly)
analysis_df <- df %>%
  mutate(
...
# ----------------------------
B <- 200  # for final manuscript you can raise to 500–1000
n <- nrow(analysis_df)
...
for (b in seq_len(B)) {
  ...
  # Refit model on bootstrap sample using same formula as 05
  fit_b <- coxph(
    Surv(time_months, event) ~ AGE + SEX + GRADE + CDCC_TOTAL_BEST +
  ...
  boot_c_app[b] <- survConcordance(Surv(time_months, event) ~ lp_b_in, data = d_b)$concordance
  
  # test C when applying bootstrap-fit to original sample
  lp_b_out <- predict(fit_b, newdata = analysis_df, type = "lp")
  boot_c_test[b] <- survConcordance(Surv(time_months, event) ~ lp_b_out, data = analysis_df)$concordance
}
...
cat("Optimism-corrected C-index:", round(c_optimism_corrected, 3), "\n")
...
# ----------------------------
# 4) Calibration at 12/36/60 months (deciles of predicted survival)
#     Approach: predicted S(t) via baseline survival * exp(lp),
#     observed S(t) via KM within risk groups.
# ----------------------------

# Build baseline survival from the fitted Cox model
# survfit(fit) gives baseline survival at centered covariates; using predict(type="risk") method is messy.
# Instead: compute predicted survival for each subject from survfit(fit, newdata=...)
# This is stable for a single dataset (no factor mismatch issues).

get_calibration_df <- function(data, horizon_months, n_groups = 10) {
  sf <- survfit(fit, newdata = data)
  
  # Extract predicted survival at horizon for each subject
  # sf$surv is matrix: time x subject
  t_index <- max(which(sf$time <= horizon_months))
  predS <- if (length(t_index) == 0) rep(NA_real_, ncol(sf$surv)) else as.numeric(sf$surv[t_index, ])
  
  tmp <- data %>%
    mutate(pred_surv = predS) %>%
    filter(!is.na(pred_surv)) %>%
    mutate(group = ntile(pred_surv, n_groups))  # low pred_surv = higher risk
  
  # Observed survival at horizon using KM within each group
  out <- tmp %>%
    group_by(group) %>%
    summarise(
      n = n(),
      pred_mean = mean(pred_surv),
      # KM estimate at horizon
      obs = {
...
plot_cal <- function(cal_df, horizon) {
  ggplot(cal_df %>% filter(horizon == horizon),
