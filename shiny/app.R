...
infer_levels_from_coef <- function(prefix, ref_level) {
  cn <- names(stats::coef(fit))
  hits <- cn[startsWith(cn, prefix)]
  if (length(hits) == 0) return(character(0))
  levs <- sub(paste0("^", prefix), "", hits)
  levs <- gsub("^", "", levs)
  levs <- gsub("^", "", levs)
  levs <- gsub("^", "", levs)
  levs <- c(ref_level, levs)
  unique(levs)
}

insurance_levels <- if (!is.null(xlev$insurance_bin4) && length(xlev$insurance_bin4) > 0) {
  xlev$insurance_bin4
} else {
  levs <- infer_levels_from_coef("insurance_bin4", "Uninsured")
  if (length(levs) == 0) c("Uninsured", "Private", "Medicaid", "Medicare_OtherGov") else levs
}

income_levels <- if (!is.null(xlev$income_lowhigh) && length(xlev$income_lowhigh) > 0) {
  xlev$income_lowhigh
} else {
  levs <- infer_levels_from_coef("income_lowhigh", "Low")
  if (length(levs) == 0) c("Low", "High") else levs
}

ins_choices <- setNames(insurance_levels, pretty_ins(insurance_levels))
inc_choices <- setNames(income_levels, pretty_income(income_levels))

sex_levels <- xlev$SEX %||% c("Male", "Female")
race_levels <- xlev$race %||% c("White", "Black", "Other")
eth_levels  <- xlev$ethnicity %||% c("Non-Hispanic", "Hispanic")
grade_levels <- xlev$grade23 %||% c("2", "3")
...
ui <- page_fluid(
  ...
  tags$head(
    tags$style(HTML("
      :root { --page-max: 1320px; --card-radius: 24px; --shadow-soft: 0 8px 28px rgba(31,52,73,0.07); --border-soft:#e7edf5; --text-main:#243447; --text-muted:#5b6b7f; --bg-soft:#f4f7fb; --accent:#1f4e79; }
      body { background: var(--bg-soft); }
      .app-container { max-width: var(--page-max); margin:0 auto; padding:24px 22px 36px 22px; }
      .app-header { background:#fff; border-radius:28px; padding:clamp(18px,2.2vw,30px); margin-bottom:24px; box-shadow:var(--shadow-soft); border:1px solid var(--border-soft); }
      ...
      .header-title { margin:0 0 8px 0; font-weight:800; line-height:1.04; font-size:clamp(2rem,3.7vw,3.4rem); color:var(--text-main); max-width:900px; }
      .ohsu-subtitle { color:var(--text-muted); margin:0 0 3px 0; font-size:1.05rem; }
      .ohsu-dept { color:#738396; margin:0; font-size:0.98rem; }
      .input-card,.metric-card,.plot-card,.detail-card { background:#fff; border:1px solid var(--border-soft)!important; border-radius:var(--card-radius)!important; box-shadow:var(--shadow-soft); }
      ...
      .form-control:focus,.form-select:focus { border-color:#8db1d5!important; box-shadow:0 0 0 0.16rem rgba(31,78,121,0.10)!important; }
      ...
      .btn-primary:hover { background-color:#1d476f!important; border-color:#1d476f!important; }
      ...
      .metric-value { font-size:clamp(1.8rem,2.4vw,2.45rem); line-height:1; font-weight:800; color:var(--accent); margin-bottom:10px; }
      .metric-label { font-size:0.96rem; color:var(--text-muted); line-height:1.35; }
      ...
      .checkbox { margin-top:6px; margin-bottom:0; }
      .detail-grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:26px 38px; }
      .detail-section { min-width:0; }
      @media (max-width:1199px){ .metric-grid{grid-template-columns:repeat(2,minmax(0,1fr));} .sticky-panel{position:static;} }
      @media (max-width:767px){ .app-container{padding:18px 14px 28px 14px;} .header-grid{grid-template-columns:1fr; gap:14px; text-align:center;} .metric-grid,.detail-grid{grid-template-columns:1fr;} .input-card .card-body,.metric-card .card-body,.plot-card .card-body,.detail-card .card-body{padding:18px;} }
    "))
  ),
  ...
  div(
    ...
    div(
      ...
      div(
        class = "header-grid",
        div(class = "logo-wrap", img(src = "ohsu_logo.png", class = "ohsu-logo")),
        div(
    ...
    layout_columns(
      ...
      div(
        div(
          ...
          card(class = "metric-card", card_body(div(textOutput("s1"), class = "metric-value"), div("1-year overall survival", class = "metric-label"))),
          card(class = "metric-card", card_body(div(textOutput("s3"), class = "metric-value"), div("3-year overall survival", class = "metric-label"))),
          card(class = "metric-card", card_body(div(textOutput("s5"), class = "metric-value"), div("5-year overall survival", class = "metric-label"))),
          card(class = "metric-card", card_body(div(textOutput("med"), class = "metric-value"), div(tags$span("Median predicted survival", class = "metric-label",
        ...
        div(
          ...
          div(
            ...
            tags$ul(
              ...
              tags$li("Treatment variables (radiotherapy and extent of surgery) were evaluated separately as a sensitivity analysis and are not inputs to this diagnosis-time calculator.")
            )
          ),
          div(
            class = "detail-section",
            ...
            tags$ul(
              ...
              tags$li("Predictors: age, tumor size, sex, race, ethnicity, Charlson-Deyo score, WHO grade, insurance category, neighborhood income group.")
            )
          ),
          div(
            class = "detail-section",
            ...
            tags$ul(
              ...
              tags$li("Continuous predictors were modeled using spline terms as specified in the final model."),
              ...
              tags$li("Internal validation used bootstrap optimism correction and horizon-specific performance assessment.")
            )
          ),
          div(
            class = "detail-section",
            ...
            tags$ul(
              tags$li("Population-level risk estimates, not deterministic patient-specific prognosis."),
              tags$li("Interpret in context of pathology, imaging, treatment details, and clinical judgment."),
              tags$li("Median predicted survival is reported only when estimable from the predicted survival function.")
            )
          )
        )
      )
...
server <- function(input, output, session) {
  ...
  newdata <- eventReactive(input$calc, {
    ...
    data.frame(
      age = age_val,
      SEX = safe_factor(input$sex, sex_levels),
      cdcc = as.numeric(input$cdcc),
      grade23 = safe_factor(input$grade, grade_levels),
      tumor_size_mm = tum,
      race = safe_factor(input$race, race_levels),
      ethnicity = safe_factor(input$ethnicity, eth_levels),
      insurance_bin4 = safe_factor(input$insurance, insurance_levels),
      income_lowhigh = safe_factor(input$income, income_levels),
      check.names = FALSE
    )
  })
  ...
  output$s1 <- renderText({ req(surv_obj()); sprintf("%.1f%%", 100 * surv_at(surv_obj(), 12)) })
  output$s3 <- renderText({ req(surv_obj()); sprintf("%.1f%%", 100 * surv_at(surv_obj(), 36)) })
  output$s5 <- renderText({ req(surv_obj()); sprintf("%.1f%%", 100 * surv_at(surv_obj(), 60)) })
  ...
  output$survplot <- renderPlot({
    ...
    ggplot(df, aes(x = time, y = surv)) +
      { if (isTRUE(input$show_ci)) geom_ribbon(aes(ymin = lower, ymax = upper), fill = "#8fb3d9", alpha = 0.22) } +
      geom_vline(data = guide_df, aes(xintercept = time), linetype = "dashed", linewidth = 0.55, color = "#cbd6e2") +
      geom_step(color = "#1f6feb", linewidth = 1.5, direction = "hv") +
      geom_point(data = pts_df, aes(x = time, y = surv), inherit.aes = FALSE, color = "#1f6feb", size = 3.2) +
      scale_x_continuous(limits = c(0, 120.8), breaks = seq(0, 120, by = 12), expand = expansion(mult = c(0.01, 0.02))) +
      scale_y_continuous(limits = c(0, 1.04), breaks = seq(0, 1, by = 0.2), labels = function(x) sprintf("%.1f", x),
